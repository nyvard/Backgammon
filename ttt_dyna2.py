...
def epsilon_nn_greedy_obs(board, player, epsilon, observed, w1, b1, w2, b2, wt1, bt1, wt2, bt2, debug=False):
    moves = legal_moves(board)
    if (np.random.uniform() < epsilon):
        if debug == True:
            print("explorative move")
        return np.random.choice(np.where(observed[board]))
    na = np.size(moves)
    va = np.zeros(na)
    for i in range(0, na):
        board[moves[i]] = player
...
def search(board, player epsilon, wt1, bt1, wt2, bt2, A, B, Observed, n):
    for param in (wt1, bt1, wt2, bt2):
        param.retain_grad()
...
def learnit(numgames, epsilon, lam, alpha, V, alpha1, alpha2, w1, b1, w2, b2):
    ...
    Observed = np.zeros(2*9) # keep track of observed boards
    A = np.zeros((2*9, 2*9)) # transition matrix (from row to column)
    B = np.zeros((2*9, 2)) # c0 has sum of rewards, c1 has #visits
    ...
    for games in range(0, numgames):
        ...
        wt1 = Variable(torch.randn(9*9,2*9, device = device, dtype=torch.float), requires_grad = True)
        bt1 = Variable(torch.zeros((9*9,1), device = device, dtype=torch.float), requires_grad = True)
        wt2 = Variable(torch.randn(1,9*9, device = device, dtype=torch.float), requires_grad = True)
        bt2 = Variable(torch.zeros((1,1), device = device, dtype=torch.float), requires_grad = True)
        ...
        for move in range(0, 9):
            ...
            if (player == tableplayer): # this one is using the table V
                ...
                Observed[one_hot_encoding(board, 1)] = True # record visit
                
                # dream on
                search(board, epsilon, w1.clone, b1.clone, w2.clone, b2.clone, A, B, Observed, n)
                # choose action
                anew = epsilon_nn_greedy(np.copy(board), player, epsilon, w1, b1, w2, b2, wt1, bt1, wt2, bt2)
                sold = one_hot_encoding(board, 1)
            ...
            if (1 < move):
                if tableplayer == player: # here we have player 1 updating the table V
                    ...
                    A[sold, one_hot_encoding(board, 1)] += 1
                    B[sold,:] += (0,1)
                    x = Variable(torch.tensor(one_hot_encoding(board, player), dtype = torch.float, device = device)).view(2*9,1)
...
for t in range(0,100):
    start = time.time()
    learnit(1000, epsilon, lam, alpha, V, alpha1, alpha2, w1, b2, w2, b2)
    end = time.time()
    print(t)
    print(end-start)
#end = time.time()
